...
//				$update="insert into level(level,order,incident_id
		...
		//new clause
		$orderSQL="select * from level where incident_id='".$incident_report."' limit 1";
		$orderRS=$db->query($orderSQL);	
		$orderRow=$orderRS->fetch_assoc();
		
		$level_order=$orderRow['order'];
		$ex_level=$orderRow['level'];

		
		$updateSQL="delete from level where incident_id='".$incident_report."'";
		$updateRS=$db->query($updateSQL);
		
		$updateSQL="update level set order=order-1 where date='".$incident_date."' and level='".$ex_level."' and order>'".$level_order."'";
		$updateRS=$db->query($updateSQL);
