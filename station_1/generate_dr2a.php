...
<?php
	...
	for($i=0;$i<$nm;$i++){	
		...
		$teemr="select * from ticket_error where ticket_daily_id='".$daily_id."' and error_code like 'JC%%' ";
		$teemr_rs=$db->query($teemr);
		$teemr_nm=$teemr_rs->num_rows;
		
		$jc_error=$teemr_nm;
		
		$teemr="select * from ticket_error where ticket_daily_id='".$daily_id."' and error_code='MTC' ";
		$teemr_rs=$db->query($teemr);
