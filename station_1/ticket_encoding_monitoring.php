...
<?php
	...
	for($i=0;$i<$nm;$i++){
		...
		if($error_cat==""){
			...
			$error_c_sql="select * from category_error where id='".$error_cat."' limit 1";
			$error_c_rs=$db->query($error_c_sql);
			
			$error_c_row=$error_c_rs->fetch_assoc();
			$error_category=$error_c_row['category'];
		}
	...
	?>
		<tr>
			...
			<td align=center><?php echo $error_code; ?> <a href='#' onclick="fillEdit('error_code','<?php echo $row['id']; ?>','<?php echo $reference_id; ?>')">Edit</a></td>
