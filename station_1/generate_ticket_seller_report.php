...
if(isset($_GET['ticket_seller'])){
	...
	for($i=0;$i<$nm;$i++){
		...
		$ticket_error_sql="select count(1) as error_count from ticket_error_daily inner join ticket_error on ticket_error_daily.id=ticket_error.ticket_daily_id where date='".$row['date']."' and station='".$row['station']."' and ticket_seller='".$ticket_seller."' and error_category='1'";
