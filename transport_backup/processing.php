...
if(isset($_GET['checkCar'])){
	...
	$sql="select * from train_availability inner join train_ava_time on train_availability.id=train_ava_time.train_ava_id where (car_a='".$_GET['checkCar']."' or car_b='".$_GET['checkCar']."' or car_c='".$_GET['checkCar']."') and remove time is null and date like '".$availability_date_code."%%'";
	$rs=$db->query($sql);
	$nm=$rs->num_rows;
	if($nm>0){
		if($_GET['checkCar']==""){
...
if(isset($_GET['removeRow'])){
	...
	if($snm>0){
		for($i=0;$i<$snm;$i++){
			...
			$update="delete from incident_report where id='".$incident."'";
			$rs=$db->query($update);
