...
//			htmlCode+=document.getElementById('td').innerHTML;
	...
	//	htmlCode+=document.getElementById('cell').innerHTML;
		...
		htmlCode+="<td><input type=text name='car_1' id='car_1' onblur='fillCar(\"first\",this.value)'  /></td>";
		htmlCode+="</tr>";
		htmlCode+="<tr>";
		htmlCode+="<td>Car 2</td>";
		htmlCode+="<td><input type=text name='car_2' id='car_2' onblur='fillCar(\"mid\",this.value)'  /></td>";
		htmlCode+="</tr>";
		htmlCode+="<tr>";
		htmlCode+="<td>Car 3</td>";
		htmlCode+="<td><input type=text name='car_3' id='car_3' onblur='fillCar(\"last\",this.value)'  /></td>";
		htmlCode+="</tr>";
		htmlCode+="<tr>";
...
?>	
		...
		<td align=center><a href='#' onclick='window.open("car_history.php?car_id=<?php echo $row['car_a']; ?>")'><?php echo $row['car_a']; ?></a></td>
	...
	<?php	
		...
		if($row['status']=="active"){
			...
			if($cancelNM>0){
				...
				for($m=0;$m<$cancelNM;$m++){
					...
					if($level==2){
						if($l2Count==0){
							$level2Clause.=getOrdinal($cancelRow['order']);
						}
						else {
							$level2Clause.=",<br>";
							$level2Clause.=getOrdinal($cancelRow['order']);
						...
						if($l3Count==0){
							$level3Clause.=getOrdinal($cancelRow['order']);
						}
						else {
							$level3Clause.=",<br>";
							$level3Clause.=getOrdinal($cancelRow['order']);
						...
						if($l4Count==0){
							$level4Clause.=getOrdinal($cancelRow['order']);
						}
						else {
							$level4Clause.=",<br>";
							$level4Clause.=getOrdinal($cancelRow['order']);
		...
		<td rowspan=3><?php echo $insert_time; ?> <a href='#add_form' onclick='changeForm("insertion","<?php echo $row['id']; ?>","<?php if($row['type']=="unimog"){ echo "unimog"; } else if($row['type']=="test"){ echo "test"; } ?>")'>Edit</a> / <a href='#' onclick='cancelTrain("<?php echo $row['id']; ?>")'>Cancel</a></td>
		<td rowspan=3><?php echo  $insert_driver; ?></td>

<!--<a href='#' onclick='changeForm("index_switch","")'>Switch Index No.</a>-->
		<td rowspan=3><?php echo $remove_time; ?> <a href='#add_form' onclick='changeForm("removal","<?php echo $row['id']; ?>","<?php if($row['type']=="unimog"){ echo "unimog"; } else if($row['type']=="test"){ echo "test"; }  ?>")'>Edit</a></td>
		<td rowspan=3><?php echo $remove_driver; ?></td>
		<td rowspan=3><?php echo $remove_remarks; ?>
		<?php echo $incidentClause; ?>
		<br><a href='#add_form' onclick='changeForm("remarks","<?php echo $row['id']; ?>","<?php if($row['type']=="unimog"){ echo "unimog"; } else if($row['type']=="test"){ echo "test"; }  ?>")'>Add/Edit Remarks </a>
...
<a href='#' style='text-underline: none;' onclick='changeForm("add_train","","")'>+Add Train</a> | <a href='#' style='text-underline: none;' onclick='changeForm("unimog","","")'>UNIMOG</a>
<br>
<br>
...
<td>
<input type=text name='car_1' id='car_1' onblur='fillCar("first",this.value)'/>
</td>
</tr>
<tr>
<td>
...
<td>
<input type=text name='car_2' id='car_2' onblur='fillCar("mid",this.value)' />
</td>
</tr>
<tr>
<td>
...
<td>
<input type=text name='car_3' id='car_3' onblur='fillCar("last",this.value)' />
</td>
</tr>
<tr>
<td>
...
<div id='td' style='visibility:hidden;'>
<select name='train_driver' id='train_driver'>
<?php
$db=new mysqli("localhost","root","","transport");
