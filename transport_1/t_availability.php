...
//		}		
	...
	//	htmlCode+=document.getElementById('cell').innerHTML;
		...
		htmlCode+="<td><input type=text name='index_no'  autocomplete='off'  /></td>";
		htmlCode+="</tr>";
		htmlCode+="<tr>";
		htmlCode+="<td>Car 1</td>";
		htmlCode+="<td><input type=text name='car_1' id='car_1' autocomplete='off' onblur='fillCar(\"first\",this.value)'  /></td>";
...
//if((isset($_POST['day']))||(isset($_SESSION['day']))){
	...
	$sql="select * from train_availability where date between '".$availability_date." 00:00:00' and '".$availability_date." 23:59:59' order by date limit 9";
		...
		if($row['status']=="active"){
			...
			if($cancelNM>0){
				...
				for($m=0;$m<$cancelNM;$m++){
					...
					$order=getLevel($cancelRow['incident_id']);
...
//						$level2Clause="&nbsp;";						
		...
		<td rowspan=3><?php echo $inserted_to.$insert_time; ?> <a href='#add_form' onclick='changeForm("insertion","<?php echo $row['id']; ?>","<?php if($row['type']=="unimog"){ echo "unimog"; } else if($row['type']=="test"){ echo "test"; } else if($row['type']=="reserve"){ echo "reserve"; } ?>")'>Edit</a> / <a href='#' onclick='cancelTrain("<?php echo $row['id']; ?>")'>Cancel</a></td>
		<td rowspan=3><?php echo $insert_driver; ?></td>

<!--<a href='#' onclick='changeForm("index_switch","")'>Switch Index No.</a>-->
		<td rowspan=3><?php echo $removed_from.$remove_time; ?> <a href='#add_form' onclick='changeForm("removal","<?php echo $row['id']; ?>","<?php if($row['type']=="unimog"){ echo "unimog"; } else if($row['type']=="test"){ echo "test"; } else if($row['type']=="reserve"){ echo "reserve"; } ?>")'>Edit</a></td>
