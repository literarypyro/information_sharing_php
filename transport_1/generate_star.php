...
//		echo $car_sql3;
		...
		if($hourLabel<=12){
			$reserve=$rRow['h_'.$hourLabel]; 
		}
		else {
			$reserve=$rRow2['h_'.$hourLabel]; 
		}
...
//		$incident_sql="select * from train_incident_view inner join incident_description on train_incident_view.incident_id=incident_description.incident_id where incident_date ".$timestamp." and level='3'";
		...
		$incident_sql="select * from incident_report inner join incident_description on incident_report.id=incident_description.incident_id where incident_date ".$timestamp." and ((incident_type in ('rolling') and level in ('3','4')) or (incident_type in ('gradual','c_loops','r_trains')))";
		...
		addContent(setRange("H".$rowCount,"L".$rowCount),$excel,$incident,"true",$ExWs);
