...
/*
		$sql="select * from train_reserve where hourly_date='".$train_date."' and hour='".$reserve_hour."'";
		$rs=$db->query($sql);
		$nm=$rs->num_rows;		
		if($nm>0){
			$row=$rs->fetch_assoc();
			$update="update train_reserve set reserve=\"".$reserve."\" where id='".$row['id']."'";
			$updateRS=$db->query($update);
		}
		else {
			$update="insert into train_reserve(hourly_date,hour,reserve) values ('".$train_date."','".$reserve_hour."',\"".$reserve."\")";
			$updateRS=$db->query($update);
		}		
*/
...
/*
		$sql="select * from train_cars_provided where hourly_date='".$train_date."' and hour='".$provided_hour."'";
		$rs=$db->query($sql);
		$nm=$rs->num_rows;		
		...
		if($nm>0){
			...
			$row=$rs->fetch_assoc();
			$update="update train_cars_provided set cars_provided=\"".$cars_provided."\" where id='".$row['id']."'";
			$updateRS=$db->query($update);
		}
		else {
			$update="insert into train_cars_provided(hourly_date,hour,cars_provided) values ('".$train_date."','".$provided_hour."',\"".$cars_provided."\")";
			$updateRS=$db->query($update);
		}		
*/
...
//			$hour=str_replace("30",":30",$hour);
			...
			$nexthour=str_replace("31",":30",$nexthour);

			$timestamp="between '".date("Y-m-d H:i:s",strtotime($timetable." ".str_replace("30",":30",$hour).":00"))."' and '".date("Y-m-d H:i:s",strtotime($timetable." ".$nexthour.":00"))."'";		
		}
...
//		$cars_cancelled=$car_nm2;
	...
	//	$cars_provided_sql="select * from train_cars_provided where hourly_date='".$timetable."' and hour='".$hour."' limit 1";
	//	$cars_provided_rs=$db->query($cars_provided_sql);
	//	$cars_provided_nm=$cars_provided_rs->num_rows;		
		?>
...
//		$reserve_sql="select * from train_reserve where hourly_date='".$timetable."' and hour='".$hour."' limit 1";
//		$reserve_rs=$db->query($reserve_sql);
//		$reserve_nm=$reserve_rs->num_rows;		
/*
		if($reserve_nm>0){
			$reserve_row=$reserve_rs->fetch_assoc();
			echo $reserve_row['reserve'];
		}
		else {
			echo "&nbsp;";
		}
