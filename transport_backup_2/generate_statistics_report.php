...
if(isset($_GET['year'])){
	...
	$rowCount=1;
	addContent(setRange("A".$rowCount,"C".$rowCount),$excel,"Year ".$year,"true",$ExWs);
	$rowCount++;
	addContent(setRange("A".$rowCount,"C".$rowCount),$excel,"Level ".$level,"true",$ExWs);
	...
	for($i=1;$i<=12;$i++){	
		...
		$end_date=date("Y-m-d",strtotime($year."-".$date_limit."-01"));
