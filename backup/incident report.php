...
if(isset($_POST['incident_no'])){
	...
	$sql.="description,action_dotc,action_maintenance,duration,equipt,cancel)";
	$sql.=" values ";
	$sql.="(\"".$type."\",\"".$incident_id."\",'".$level."','".$incident_date."',";
	$sql.="\"".$description."\",\"".$dotc_taken."\",\"".$maintenance_taken."\",\"".$duration."\",'".$equipment."','".$cancel."')";
	...
	if($level==2){
		...
		$update="insert into level(date,level,incident_id) values ('".$incident_date."','2','".$incident_code."')";
		$rs=$db->query($update);
		...
		$update="insert into level(date,level,incident_id) values ('".$incident_date."','3','".$incident_code."')";
		$rs=$db->query($update);
		...
		$update="insert into level(date,level,incident_id) values ('".$incident_date."','4','".$incident_code."')";
		$rs=$db->query($update);
...
<tr>
<td>On-board Equipt. or Accessories?</td>
<td>
...
<input  name='index_id' id='index_id' style='border: 1px solid gray' type=text size=5 <?php if((isset($_GET['cancel']))||(isset($_GET['add_incident']))){ } else { echo "disabled"; } ?> value='<?php echo $index_id; ?>' /> <span style='color: rgb(0,51,153);' >/</span> 
...
<tr>
<td>Incident No.</td><td> <input type=text name='incident_no' />
</td>
</tr>
<tr>
...
<?php
$db=new mysqli("localhost","root","","transport");
$sql="select * from train_driver where position='STDO' order by lastName";
$rs=$db->query($sql);
$nm=$rs->num_rows;
for($i=0;$i<$nm;$i++){
	$row=$rs->fetch_assoc();
...
<tr>
<td valign=top>DOTC:</td><td><span name='remarks_space' id='remarks_space'> <textarea rows=5 cols=50 name='dotc'></textarea></span><input type=checkbox name='remarks_check' id='remarks_check' onclick='setPreset(this)' /><font color=blue>Preset Values</font></td></tr>
<tr>
