...
if(isset($_GET['ccdr'])){
	...
	for($i=0;$i<$nm;$i++){
		...
		//added condition level alert color 3 and 4 mjun
		if($level==3){			
			cellColor(setRange("A".$rowCount,"N".$rowCount),$excel,$level);
			//cellColor("N".$rowCount,$excel,$level);		
		}
		elseif($level==4) {
			cellColor(setRange("A".$rowCount,"N".$rowCount),$excel,$level); 
			//cellColor("N".$rowCount,$excel,$level);			
		...
		//initialize serve as page break control records
		if($page_counter==14) {	
