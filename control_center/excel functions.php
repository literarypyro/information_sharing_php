...
// Set color change mjun
function cellColor($range,$excelAPI,$level)
{        

	$cell=$range[0].':'.$range[1];
	//$excelAPI->getActiveSheet()->mergeCells($range[0].':'.$range[1]);
	
    $excelAPI->getActiveSheet()->getStyle($cell)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
    
    //green
	//$excelAPI->getActiveSheet()->getStyle($cell)->getFill()->getStartColor()->setARGB('1E90FF');
	if ($level==3) {
		//yellow color
		$excelAPI->getActiveSheet()->getStyle($cell)->getFill()->getStartColor()->setARGB('FFFFFF00'); 	
	}
	else {
		//red
		$excelAPI->getActiveSheet()->getStyle($cell)->getFill()->getStartColor()->setARGB('FFFF0000');
	}
}
