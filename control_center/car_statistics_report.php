...
$sql="SELECT car_no,month(incident_date) as mo,sum(1) as count FROM incident_cars inner join incident_report on incident_cars.incident_id=incident_report.id where incident_date like '".$year."-%%' group by car_no*1,month(incident_date)";
$rs=$db->query($sql);
...
<th><a href='#' onclick='window.open("car_history.php?car_id=<?php echo $i; ?>")'><?php echo $i; ?></a></th>
<?php
for($k=1;$k<=12;$k++){
?>			
