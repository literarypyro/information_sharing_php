...
<form action='edit_ccdr.php' method='post'><font face="verdana" size="2" color="black"><b>Search Incident Number </b> </font><input class='text_input' type=text name='search_incident_number' style="height: 20px" /><input type=submit value='Search' /></form>
...
if(isset($_POST['search_incident_number'])){
	...
	if($nm>0){
	?>	
<form action='edit_ccdr.php' method=post><font face="verdana" size="2" color="black"><b>Retrieve Incident Report </b></font><select  class='text_input' name='incident_report' style="height: 20px">
	<?php
		for($i=0;$i<$nm;$i++){
		$row=$rs->fetch_assoc();
	?>	
...
<tr><th width=20%>Incident Number</th><td width=50%><?php echo $incident_no; ?></td><td width=5%></td>
...
<form id='edit_form' name='edit_form' action='edit_ccdr.php'  method='post'>
	...
	<br>
	<div align=left><font color=white>| | | | | | | | | | | | | | | | | | | |</font><input type="hidden" name='fieldType' id='fieldType' /> <input type="hidden" name="inc_report" value='<?php echo $incident_report?>' /> <input type=submit name='submit' id="Suc" value='Edit' /></div>
	</form>
