...
function retrieveOfficerListHTML2($db,$officer_name,$division){
	...
	for($i=0;$i<$nm;$i++){
		$row=$rs->fetch_assoc();
		?>
		<option <?php if($row['division']==$division){ echo "selected"; } ?> data-division='<?php echo $row['division']; ?>' value='<?php echo $row['id']; ?>'><?php echo $row['name']; ?></option>
	<?php
	}
	...
	for($i=0;$i<$nm2;$i++){
		...
		?>	
		<option data-name='<?php echo $row2['firstName']." ".$row2['lastName']; ?>' data-position='<?php echo $row2['position']; ?>' value='OTHER' ><?php echo $row2['firstName']." ".$row2['lastName']; ?></option>
	<?php
	}
	?>	
	<!--
